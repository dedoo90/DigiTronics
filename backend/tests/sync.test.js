...
describe('sync queue push contract', () => {
  test('POST with a client-supplied id stores the record under that id', async () => {
    ...
    const get = await request(server.baseUrl).get('/api/v1/customers/local-abc-123');
  ...
  test('records are resolvable through their _backendId alias', async () => {
    ...
    const byLocal = await request(server.baseUrl).get('/api/v1/customers/local-xyz');
    const byBackend = await request(server.baseUrl).get('/api/v1/customers/backend-xyz');
    expect(byLocal.statusCode).toBe(200);
    expect(byBackend.statusCode).toBe(200);
    expect(byBackend.body.data.id).toBe('local-xyz');
    const upd = await request(server.baseUrl)
...
describe('refresh merge contract', () => {
  test('records added externally to the store appear in subsequent list calls', async () => {
    const before = await request(server.baseUrl).get('/api/v1/suppliers');
    ...
    const after = await request(server.baseUrl).get('/api/v1/suppliers');
  ...
  test('API-created records and externally seeded records coexist (unsynced preserved)', async () => {
    await request(server.baseUrl).post('/api/v1/employees').send({ id: 'api-made', name: 'API Made' });
    const list = await request(server.baseUrl).get('/api/v1/employees');
    ...
    const merged = await request(server.baseUrl).get('/api/v1/employees');
...
describe('conflict detection basis', () => {
  ...
  test('server applies last-write-wins on conflicting updates', async () => {
    ...
    await request(server.baseUrl).put(`/api/v1/customers/${id}`).send({ phone: '0444' });
    const second = await request(server.baseUrl).put(`/api/v1/customers/${id}`).send({ phone: '0555' });
    expect(second.statusCode).toBe(200);
    const final = await request(server.baseUrl).get(`/api/v1/customers/${id}`);
